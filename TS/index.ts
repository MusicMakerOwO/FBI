...
import './Utils/CheckPackages.js';

import MessageTypes from './Utils/Sharding/MessageTypes.js'; // enum for sharding communication

import Prompt from './Utils/Prompt.js';
import CRC32 from './Utils/crc32.js';
import ComponentLoader from './Utils/ComponentLoader.js';
import ReadFolder from './Utils/ReadFolder.js';

// We don't want to run this on the bot instance or it will run for each and every shard lol
import RegisterCommands from './Utils/RegisterCommands.js';
import FileWatch from './Utils/FileWatch.js';

import type { RESTPostAPIChatInputApplicationCommandsJSONBody } from 'discord.js';
import type { CommandFile, ContextFile, IPCMessage } from './typings.js';
...
async function MakeRequest(method: string, route: string, body: any) {
...
function StringifyFunction(key: string, value: any) {
	switch (typeof value) {
		case 'function':
			return null;
		case 'bigint':
...
function SimplifyCommand(command: CommandPayload) : CommandPayload {
	return {
		name: command.name,
...
function TokenizeCommand(command: CommandPayload) {
	return CRC32(JSON.stringify(command, StringifyFunction));
}

function CheckCommandEquality(oldCommand: CommandPayload, newCommand: CommandPayload) {
	const oldToken = TokenizeCommand(oldCommand);
	const newToken = TokenizeCommand(newCommand);
	return oldToken === newToken;
}

function NeedsRegister(oldCommands: Record<string, CommandPayload>, newCommands: Record<string, CommandPayload>) {
...
async function DynamicRegister() {
	...
	const oldCommands: Record<string, CommandPayload> = {};
	const newCommands: Record<string, CommandPayload> = {};

	const oldDevCommands: Record<string, CommandPayload> = {};
	const newDevCommands: Record<string, CommandPayload> = {};
	...
	const currentFiles = ReadFolder(`${__dirname}/commands`);
	for (const file of currentFiles) {
		delete require.cache[require.resolve(file.path)];
	}

	const components = {
		commands: new Map<string, CommandFile>(),
		context: new Map<string, ContextFile>()
	}
	...
	for (const [name, command] of components.commands.entries()) {
		if (command.dev) {
			newDevCommands[name] = SimplifyCommand(command.data as unknown as CommandPayload);
		} else {
			newCommands[name] = SimplifyCommand(command.data as unknown as CommandPayload);
		}
	}
	...
	const registeredCommands = await MakeRequest('GET', `https://discord.com/api/v10/applications/${config.APP_ID}/commands`, null) as CommandPayload[]; // Array
	...
	if (!needsRegister && config.DEV_GUILD_ID.length > 0) {
		const registeredDevCommands = await MakeRequest('GET', `https://discord.com/api/v10/applications/${config.APP_ID}/guilds/${config.DEV_GUILD_ID}/commands`, null) as CommandPayload[]; // Array
	...
	if (needsRegister) {
		...
		// @ts-ignore - Again expecting a client lol
...
async function GetGuildCount() {
	const guilds = await MakeRequest('GET', `https://discord.com/api/v10/users/@me/guilds`, null) as any[];
...
function CreateShard(shardID: number, shardCount = shards.size) {
...
function GetLatestLogFile() {
	...
	const dates = logs.map(log => log.split('.').shift()).sort((a, b) => new Date(b as string).getTime() - new Date(a as string).getTime());
...
function AppendLogs(type: string, shardID: number, message: string, silent = false) {
...
function ForceKill(shardID: number) {
...
// IPC relays
function ResetTimeout(requestID: string) {
...
function ProcessIPCMessage(message: IPCMessage) {
	...
	*/
	...
	if (typeof requestID !== 'string' || requestID.length === 0) {
		shard.send({ type: MessageTypes.IPC_UNKNOWN_REQUEST_ID, requestID });
		return;
	}
